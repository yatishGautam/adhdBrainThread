...
export function NowPanel({
	...
	return (
		<Panel
			title="Now"
			subtitle={
				state ? undefined : "The one thing you are working on right now."
			}
		>
			{!state ? (
				readOnly ? (
					...
					<ThreadPicker
						onPick={(id) =>
							window.thread.invoke["session:start"]({ threadId: id }).then(() =>
								setPicking(false),
							)
						}
				...
				<div style={{ display: "flex", alignItems: "center", gap: 16 }}>
					...
					<div style={{ display: "flex", gap: 6 }}>
						...
						<Button
							...
							onClick={() =>
								void window.thread.invoke["session:distraction"]({})
							}
						>
							I got distracted
						</Button>
